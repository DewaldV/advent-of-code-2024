...
fn solve_eq(total: usize, values: Vec<usize>) -> Option<usize> {
    let available_ops = vec![Operation::ADD, Operation::MULTIPLY, Operation::CONCAT];
    ...
    for ops in combinations {
        ...
        for (idx, op) in ops.iter().enumerate() {
            match *op {
                ...
                Operation::CONCAT => sum = format!("{}{}", sum, values[idx + 1]).parse().unwrap(),
            }
...
mod tests {
    ...
    fn test_example_file() {
        run_example_file(7, 11387, &solve);
    }
}
